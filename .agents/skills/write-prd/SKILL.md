---
name: write-prd
description: Write a concise PRD markdown file that captures the feature discussed in the current conversation, following this repo's PRD conventions. Use when the user asks to write, draft, or generate a PRD, product requirements doc, or spec for the work being discussed.
---

# Write a PRD

Turn the **full context of the current conversation** into one precise, concise PRD markdown file that follows this repo's existing PRD style.
...
Do **not** include an "Open questions" section. The PRD must present a fully decided plan with no unresolved questions — resolve ambiguity with the user before writing (see "Before writing").

Optional trailing `## Reference:` sections (tables, mermaid diagrams) are fine when they clarify — keep them short.

## Section guidance

- **Metadata table** — mirror fields used by sibling PRDs; keep only relevant rows. Always state which packages change vs stay unchanged.
