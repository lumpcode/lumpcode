...
| --- | --- | --- |
| **Promote** | Remove the idea entry | Create `.lumpcode/lumps/backlog/backlogItems/todo/<finalName>/desc.yml` with at least `name` and `task`. Carry over IDEA `priority` when set (same meaning: lower = sooner). Optional: `workflow` (`tdd` | `directImpl` | `manual`; omit ≡ `tdd`; `manual` = do it outside Lumpcode, always ignored), `dependsOn`, `manualReq` (omit ≡ human requirements; set `manualReq: false` when the agent should draft `requirements.md`), `requirements.md`. Final `name` may differ from the IDEAS id (prefer kebab-case). |
