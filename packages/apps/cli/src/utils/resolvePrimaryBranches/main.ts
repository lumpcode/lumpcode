...
/** Primary-branch fields from merged project+local (or a local-only / test partial). */
export type PrimaryBranchConfig = Pick<
    ResolvedProjectLocalConfig,
    'primaryBranch' | 'primaryBranches' | 'projectBaseBranch'
>;
...
const deprecatedWarnedConfigs = new WeakSet<object>();

function warnDeprecatedProjectBaseBranch(
    localConfig: PrimaryBranchConfig,
...
export function resolvePrimaryBranches(
    localConfig: PrimaryBranchConfig,
    logger?: Pick<Logger, 'warn'>,
): string[] {
