...
const DEFAULT_DISCOVERY_BRANCH = 'main';
...
const inputSchema = z.object({
    options: baseCommandOptionsSchema.extend({
        ...
        discoveryBranch: z
            .string()
            .optional()
            .describe('Initial `discoveryBranch` written to .lumpcode/local.json (default: main)'),
    }),
    arguments: z.object({}),
});
...
const handlerMaker: CommandHandlerMaker<Injections, Input, Output> = () => async (input) => {
    ...
    const localConfig = {
        ...
        discoveryBranch: input.options.discoveryBranch?.trim() || DEFAULT_DISCOVERY_BRANCH,
