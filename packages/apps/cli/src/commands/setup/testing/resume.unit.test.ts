...
describe('setup resume and dedicated worker', () => {
    ...
    async function seedExisting(input: {
        mode?: 'shared' | 'dedicated';
        local?: boolean;
        lump?: boolean;
    } = {}) {
        const writeLocal = input.local !== false;
        const writeLump = input.lump !== false;
        const mode = input.mode ?? 'dedicated';
        ...
        if (writeLump) {
            await fs.writeFile(
                path.join(dir, 'lumps', 'myFirstLump', 'config.json'),
                `${JSON.stringify(KEEP_STUB, null, 2)}\n`,
            );
        }
    ...
    it('is a no-op when local.json is valid and a lump already exists', async () => {
        await seedExisting({ mode: 'dedicated' });
        const beforeLocal = await fs.readFile(path.join(projectRoot, '.lumpcode', 'local.json'), 'utf-8');
        ...
        expect(await fs.readFile(path.join(lumpcode, 'local.json'), 'utf-8')).toBe(beforeLocal);
        ...
        expect(result.data.messages.join('\n')).toMatch(/already set up/i);
        expect(result.data.messages.join('\n')).not.toContain(WORKER_URL);
    });

    it('writes local.json and can start a dedicated worker when a lump exists but local.json does not', async () => {
        await seedExisting({ local: false });
        const result = await runSetup(
            trackingPrompter({
                select: async ({ message, choices }) => (/mode/i.test(message) ? 'dedicated' : choices[0]!.value),
            }),
        );
        expect(result.success).toBe(true);
        ...
        expect(local).toMatchObject({ mode: 'dedicated' });
        expect(JSON.parse(await fs.readFile(path.join(projectRoot, '.lumpcode', 'lumps', 'myFirstLump', 'config.json'), 'utf-8'))).toEqual(
            KEEP_STUB,
        );
        expect(pauses).toBe(0);
        expect(selects.some((m) => /commit|push/i.test(m))).toBe(false);
    ...
    it('merges extra local.json keys when the file exists but is not yet valid', async () => {
        await seedExisting({ local: false });
        await fs.writeFile(
            path.join(projectRoot, '.lumpcode', 'local.json'),
            `${JSON.stringify(EXTRA_LOCAL, null, 2)}\n`,
        );
        const result = await runSetup(
            trackingPrompter({
