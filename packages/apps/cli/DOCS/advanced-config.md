...
```pseudocode
pre-flight ─ pull primary discovery branch into the resolved workspace
...
The pre-flight, per-lump fetch/branch, and per-lump teardown are all generated by the CLI — they are not configurable in lump config. The baseline lifecycle ties to the **primary discovery branch** from [local-config.md](./local-config.md); lump-level `baseBranch` only overrides the per-lump fetch, not the teardown switch-back.
