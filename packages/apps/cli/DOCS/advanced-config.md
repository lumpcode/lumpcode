...
The pre-flight, per-lump fetch/branch, and per-lump teardown are all generated by the CLI — they are not configurable in lump config. Pre-flight mechanics and branch defaults are defined in [concepts.md § Pre-flight and modes](./concepts.md#pre-flight-and-modes) and [§ Branch resolution](./concepts.md#branch-resolution); both the per-lump fetch and the teardown switch-back target the lump's resolved `baseBranch`.
