...
This page is the **mental model** for Lumpcode CLI: **agent loop campaigns** (called **lumps**), contexts, status, how one run flows through git, and when to use `lumpcode run` vs `lumpcode start`. If you think of working this way as **loop engineering** (designing the loop that prompts your agent instead of prompting it yourself), a lump is one such loop, configured declaratively. Agent work is reviewed through PR merge. It is the clean way to start AI loops for the SDLC on an existing git repo. Tutorial: [get-started.md](./get-started.md). Field reference: [lump-config.md](./lump-config.md). Commands: [commands.md](./commands.md).
...
| **mode**        | `shared` or `dedicated` (in `.lumpcode/local.json`). Shared `run` rehearses in place on this checkout. Dedicated is the worker (pre-flight reset, `lump/…`). |
...
| ------- | -------------------- | ------- |
| **Project workspace** | `projectRoot` | Source checkout where `.lumpcode/` lives. Same folder as the execution workspace. |
| **Execution workspace** | *(CLI only)* | Git repo root Lumpcode runs in. Both modes: this checkout (`getExecutionWorkspacePath` returns `sourceProjectRoot`). |
| **Branch workspace** | `workspacePath` on `CommandFn` / `SetupWorkspaceFn` | Where the agent runs. Shared `run`: current `HEAD`. Dedicated `checkout`: equals the execution workspace. Dedicated `worktree`: a linked tree under `.lumpcode/worktrees/<branch>/`. |

```text
shared mode:
  project = execution = this checkout
  branch workspace      current HEAD (no lump/… branch)
...
**Shared** — load and run on this checkout (no preflight, no `lump/…`, no auto commit/push):

```mermaid
flowchart TD
  start["lumpcode run myLump"]
  load["Load config from this checkout + disabled"]
  contexts["Context source + status → todo list"]
  branch["branchFn = current HEAD"]
  setup["auto: lock this checkout<br/>in-place workspace"]
  loop["Per context: setup → steps → teardown<br/>git add/commit/push no-op"]
  review["TTY: c stamps LUMP markers / e exits dirty"]
  teardown["auto: workspace teardown + unlock"]
  refresh["Refresh contextStatusRecord.json"]
  start --> load --> contexts --> branch --> setup --> loop --> review --> teardown --> refresh
```
...
1. **Dedicated** resolves a concrete discovery branch and preflights the checkout there **before** config load; **shared** loads config from this checkout and does **not** preflight. Dedicated uses fetch / switch / hard-reset (not `git pull`). See [Pre-flight and modes](#pre-flight-and-modes).
2. Context discovery and the status-driven todo list run **before** branch workspace setup. Author hooks (`setupFn`, `promptFn`, …) sit inside the per-context loop after setup; details only in [advanced-config.md](./advanced-config.md#hook-lifecycle). Shared review (`c` / `e`) runs in `commands/run` after a successful walk, not in the engine.
3. Execution workspace is this checkout in both modes. See [Three workspaces](#three-workspaces).
...
**Tick behavior:** discover loadable configs (one locked discover per primary-branch scan), apply include/exclude, soft-skip disabled lumps at run time, then run the dedicated per-lump path for each match (optionally in parallel under worktree + `maxParallelRun`). There is no shared tick (`start` is dedicated-only). Hook order: [advanced-config.md § Hook lifecycle](./advanced-config.md#hook-lifecycle).
...
**Shared** `run` does not preflight. It stays on the current named branch. A dirty tree is allowed. Detached HEAD fails before any agent write.

After dedicated pre-flight, each lump prepares git inside the execution workspace according to `local.json.workspaceStrategy` (default `checkout`):
...
Shared `run` ignores `workspaceStrategy` (one checkout, current `HEAD`).

| `local.json.mode` | Execution workspace | Use when |
| ----------------- | ------------------- | -------- |
| `shared` | This checkout (`HEAD`) | Laptop rehearsal. Dirty allowed. No auto commit or push. After a successful walk, TTY `[c]` / `[e]`. `start` fails. |
| `dedicated` | This checkout | Worker you do not develop on; pre-flight runs the destructive in-place reset |

Worktrees live under the execution workspace on a dedicated worker. See [local-config.md](./local-config.md#workspace-strategies).
