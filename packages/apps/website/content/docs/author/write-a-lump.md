...
On your laptop, `project-setup` left `mode: "shared"`. `lumpcode run` rehearses on this branch (commit or stash first). Open that branch as a PR. A worker later cuts `lump/…` branches. Merge, and the next run skips that context.
