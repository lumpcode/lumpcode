...
> **Important:** `@lumpcode/core` is the engine only. For **agent loop campaign** management (project bootstrap, `.lumpcode/lumps/` configs, background daemon, status, and cleanup), use the [@lumpcode/cli](https://www.npmjs.com/package/@lumpcode/cli) package (`npm install -g @lumpcode/cli`).
...
| `@lumpcode/core` | [npm](https://www.npmjs.com/package/@lumpcode/core) | Engine API (`runLump`) — this package |
