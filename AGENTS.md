...
- One daemon model: every `start` discovers like today’s former global (dedicated: all primary-branch subticks), then applies optional `--include` / `--exclude` (`*` globs). Identity is **`daemonId`** (`~/.lumpcode/daemons/<project>.<daemonId>.daemon.*`); default unfiltered id `global` (reserved for unfiltered). Overlapping daemons allowed; start gates on unique id + corrupt-meta fail-closed only
