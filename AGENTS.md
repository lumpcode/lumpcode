...
- Tick (every daemon, dedicated): loop `primaryBranches` → locked discover per branch into one eligible queue (record `scanBranch` per `lumpName`; skip branch failures / duplicate names) → apply `--include`/`--exclude` → `runLumpQueueWithConcurrency`; pass queued scan branch as `effectiveDiscoveryBranch` into `runLumpFromLumpName` so phase 1 can load branch-only configs after the checkout moved
- Daemon identity: always `<project>.<daemonId>.daemon.*` (default unfiltered id `global`); companions default to `global` with legacy bare-path read fallback; start gates on id uniqueness + corrupt meta only (overlapping filters allowed); `--lumpName` on start/companions deprecated
