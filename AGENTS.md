...
- **`--discoveryBranch`** on `run` / `lump-plan` / `lump-status` (not on `start`); **`resolveEffectiveDiscoveryBranch`** (discovery only, not `resolvedBaseBranch`); shared warn-and-ignore
...
- `start` detaches by default; companions: `stop`, `restart`, `daemon-log`, `daemon-status` (`--daemonId`, default `global`). `daemon-status` with no id flags **lists all** project daemons
- PID/meta JSON written **only in `--foreground`** (detached parent spawns foreground child with `--daemonId` + filters). Meta writes `daemonId`, `include`/`exclude`, optional `maxParallelRun`; never writes `lumpName` (read-only compat)
...
- Tick (dedicated): per primary-branch subtick → locked discover → include/exclude → run or no-op (empty match stays up); pass `effectiveDiscoveryBranch` into `runLumpFromLumpName`. No `ignoredByGlobalDaemon` (removed)
...
- Alive PID + bad meta: `listRunningProjectDaemons` returns `{ meta: 'missing'|'invalid' }` keyed by `daemonId` (legacy bare `<project>.daemon.pid` maps to `global`); `assertDaemonStartAllowed` / `start` refuse with `daemonMetaCorrupt`; `restart` uses `stop --force` then start replaying filters from meta
- Foreground daemon arms native SIGINT/SIGTERM shutdown **before** the first `runTick`; meta mid-run writers bump **`inFlightLumpCount`** per lump in `runOneLump` `try/finally`; `busy` is deprecated read-only
