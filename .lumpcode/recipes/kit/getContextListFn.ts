...
async function loadPendingTodoContexts({ baseBranch, lumpName }: { baseBranch: string, lumpName: string }) {
    const todoPath = path.join('.lumpcode', 'lumps', lumpName, 'TODO.yaml');
    ...
    for (const item of iterateTodoItems(doc)) {
        ...
        if (itemType === 'feature') {
            const prdFilePath = path.join('.lumpcode', 'lumps', lumpName, 'prds', `${item.name}.prd.md`);
            const testPlanFilePath = path.join('.lumpcode', 'lumps', lumpName, 'testPlans', `${item.name}.test.md`);
            const nextFlow = await getFeatureNextFlow(item, baseBranch);
...
async function getFeatureNextFlow(item: TodoYamlItem, baseBranch: string) {
...
export async function getContextListFn(baseBranch: string) {
    return loadPendingTodoContexts({ baseBranch, lumpName });
}
