...
4. JSON `contextListJson` cannot declare a static `ContextList`; a literal path template plans to nothing.
...
4. `contextListJson` accepts a static `ContextList` array (or file). A no-placeholder template object fails at resolve (`{}` is an empty plan).
...
| Static `contextListJson` | `resolveGetContextListFn` + `lumpConfig.schema.json` |
...
| 2 | `static-context-list-json` | Static `ContextList` in `contextListJson` | `tickets/static-context-list-json/requirements.md` |
...
5. A static `README` `ContextList` plans to one legal context. js/ts stub matches suffix files.
6. Shared never calls `launchStartDaemon`. Skill / authoring-pkg install failures do not abort the drive.
7. First PR / get-started / commands / worker / `contextListJson` docs match. No second scaffold or ContextList parser outside the named owners.
