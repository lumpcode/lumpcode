...
const ABSTRACTION_PROMPT = `
    ...
    - Materializes the abstraction as a new util under packages/apps/cli/src/utils/<utilName>/ following existing conventions: main.ts (implementation), index.ts (re-export), unit.test.ts (unit tests), and a barrel export from packages/apps/cli/src/utils/index.ts.
    - Actually shrinks the codebase: after adding the util and refactoring all call sites in packages/apps/cli to import it, net line count must go down (removed duplication minus new util code, exluding the new unit test file). Do not extract one-off logic or move code without deleting repetition.

    Apply the refactor. The new util must include unit tests in unit.test.ts (match sibling utils in packages/apps/cli/src/utils/).
