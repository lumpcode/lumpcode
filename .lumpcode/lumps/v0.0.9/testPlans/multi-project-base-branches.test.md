...
| --- | --- | --- |
| Config helpers | `resolveProjectBaseBranches/unit.test.ts`, `validateLumpBaseBranchAllowlist/unit.test.ts`, `readLocalConfig/unit.test.ts` | Effective list resolution, primary branch fallback, parse validation, allowlist |
...
| `LC-MULTI` | `{ "mode": "dedicated", "projectBaseBranch": "main", "projectBaseBranches": ["main", "ver/0.0.9"] }` | Array wins for effective list; singular optional when array set |
| `LC-MULTI-ARRAY-ONLY` | `{ "mode": "dedicated", "projectBaseBranches": ["main", "ver/0.0.9"] }` | Valid without singular; primary branch is `main` (first element) |
...
| Array only (`LC-MULTI-ARRAY-ONLY`) | singular omitted | `["main", "ver/0.0.9"]` |
| Array order preserved (`LC-MULTI-ORDER`) | | `["ver/0.0.9", "main"]` exactly |
| Primary branch fallback | `LC-MULTI-ARRAY-ONLY` or singular omitted | `resolvePrimaryProjectBaseBranch` → first element of effective list (`"main"`) |

Export from `utils/index.ts`. Pure functions; no I/O. **`resolvePrimaryProjectBaseBranch`** — `localConfig.projectBaseBranch ?? resolveProjectBaseBranches(localConfig)[0]` (callers use this wherever a single integration branch is needed, e.g. default pre-flight `targetBranch`, lump `baseBranch` fallback).
...
| Accepts array-only config (`LC-MULTI-ARRAY-ONLY`) | success without `projectBaseBranch` on `data` |
...
| Fails when both singular and array absent | `{ "mode": "dedicated" }` → `success: false`; message mentions `projectBaseBranch` or `projectBaseBranches` |
| Fails when projectBaseBranch is missing | only fails when `projectBaseBranches` is also absent or empty |
...
| `it()` | Expectation |
| --- | --- |
| Default (no `targetBranch`) | dedicated: checkout stays/switches to primary integration branch (`projectBaseBranch` or first `projectBaseBranches` element) |
...
| `it()` | Expectation |
| --- | --- |
| `localConfig.schema.json` | optional `projectBaseBranches` array of strings; `projectBaseBranch` optional when non-empty array set; refine requires at least one |
...
| Path | Export |
| --- | --- |
| `utils/resolveProjectBaseBranches/main.ts` | `resolveProjectBaseBranches(localConfig): string[]`; `resolvePrimaryProjectBaseBranch(localConfig): string` |
...
| --- | --- | --- |
| 1 | Effective list — array wins; singular-only unchanged; array-only uses array | §1, §2, `LC-SINGLE` / `LC-MULTI` / `LC-MULTI-ARRAY-ONLY` |
| 2 | Parse validation — empty array, duplicates, neither field; array-only accepted | §2 |
...
- Array-only `projectBaseBranches` parses without singular; primary branch is the first array element (`LC-MULTI-ARRAY-ONLY`).
